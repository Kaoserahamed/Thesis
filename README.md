...
Sequence-length and loss-component ablations anchored to the table above are tabulated in
[`results/ablation_summary.csv`](results/ablation_summary.csv), generated by
[`analysis/05_ablation_summary.py`](analysis/05_ablation_summary.py):

```bash
python analysis/05_ablation_summary.py  # regenerates results/ablation_summary.csv
```

Longer temporal context monotonically improves IoU at every resolution, and the combined
`BCE+Dice` objective outperforms either term alone. Error columns (`false_positive_rate`,
`false_negative_rate`, `mean_instability`) extend the difference-overlay plot in
`utils/visualization_utils.py` with a per-variant FP/FN breakdown.
