...
The IoU / Dice / area-difference helpers are pure NumPy and live in
``utils.metrics_numpy``, so this module runs in the **fast** CI lane with no
TensorFlow installed.  Only the differentiable Keras ``dice_coefficient`` is
marked ``requires_tensorflow``.
"""
...
def test_dice_coefficient_perfect():
    ...
    from utils.model_utils import dice_coefficient
...
@pytest.mark.requires_tensorflow
def test_model_utils_reexports_numpy_metrics():
    """``utils.model_utils`` must keep re-exporting the NumPy metric API."""
    model_utils = pytest.importorskip("utils.model_utils", reason="TensorFlow not installed")

    for name in ("iou_np", "dice_np", "calculate_area_difference", "BINARY_THRESHOLD"):
        assert hasattr(model_utils, name), f"utils.model_utils lost '{name}'"

    assert model_utils.BINARY_THRESHOLD == BINARY_THRESHOLD
    assert model_utils.iou_np is iou_np
